...
public static class DI
{
    ...
    private static void EnsureCompatibilityColumns(DataContext dataContext)
    {
        ...
        var commands = new[]
        {
            ...
            "IF OBJECT_ID(N'[dbo].[InventoryTransaction]', N'U') IS NOT NULL AND COL_LENGTH(N'dbo.InventoryTransaction', N'PendingManufacturerSerialNumbersJson') IS NULL ALTER TABLE [dbo].[InventoryTransaction] ADD [PendingManufacturerSerialNumbersJson] nvarchar(max) NULL;",
            ...
            "IF OBJECT_ID(N'[dbo].[CashTransactionPayment]', N'U') IS NOT NULL AND COL_LENGTH(N'dbo.CashTransactionPayment', N'ReversalOfPaymentId') IS NULL ALTER TABLE [dbo].[CashTransactionPayment] ADD [ReversalOfPaymentId] nvarchar(50) NULL;",
            "IF OBJECT_ID(N'[dbo].[CashTransactionPayment]', N'U') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'UX_CashTransactionPayment_ActiveReversal' AND [object_id] = OBJECT_ID(N'[dbo].[CashTransactionPayment]')) CREATE UNIQUE INDEX [UX_CashTransactionPayment_ActiveReversal] ON [dbo].[CashTransactionPayment] ([ReversalOfPaymentId]) WHERE [IsDeleted] = 0 AND [ReversalOfPaymentId] IS NOT NULL;",
