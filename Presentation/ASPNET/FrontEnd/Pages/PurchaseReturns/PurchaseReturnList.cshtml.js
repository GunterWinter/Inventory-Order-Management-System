const App = {
    setup() {
        ...
        const services = {
            ...
            getSecondaryData: async (purchaseReturnId) => {
                try {
                    const response = await AxiosManager.get('/PurchaseReturn/GetSourceLineList?purchaseOrderId=' + encodeURIComponent(state.purchaseOrderId ?? '')
                        + '&purchaseReturnId=' + encodeURIComponent(purchaseReturnId ?? ''), {});
                    return response;
                } catch (error) {
                    throw error;
                }
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Sửa phiếu trả hàng mua';
                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    rowSelecting: () => {
                        ...
                        if (args.rowData) {
                            ...
                            state.mainTitle = 'Xem tr\u1ea3 h\u00e0ng mua';
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            const result = await Swal.fire({ icon: 'warning', title: 'Xác nhận xóa', text: `Bạn có chắc chắn muốn xóa ${selected.length} phiếu trả hàng đã chọn không?`, showCancelButton: true, confirmButtonText: 'Xóa', cancelButtonText: 'Hủy', heightAuto: false }); if (!result.isConfirmed) return;
                            for (const record of selected) await services.deleteMainData(record.id, StorageManager.getUserId()); await methods.populateMainData(); mainGrid.refresh(); Swal.fire({ icon: 'success', title: 'Đã xóa', text: `Đã xóa ${selected.length} phiếu trả hàng.`, heightAuto: false }); return;
                        ...
                        if (args.item.id === 'PrintPDFCustom') {
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                window.open('/PurchaseReturns/PurchaseReturnPdf?id=' + encodeURIComponent(selectedRecord.id ?? ''), '_blank', 'noopener');
                            }
                        }
                    }
                });
            ...
            create: async (dataSource) => {
                secondaryGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'warehouseStock', headerText: 'Tồn kho hiện tại', width: 140, allowEditing: false, type: 'number', format: 'N6', textAlign: 'Right' },
