                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    recordDoubleClick: async (args) => {
                        if (args.rowData) {
                            ...
                            state.mainTitle = 'Xem phi\u1ebfu giao h\u00e0ng';
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            const result = await Swal.fire({ icon: 'warning', title: 'Xác nhận xóa', text: `Bạn có chắc chắn muốn xóa ${selected.length} phiếu giao hàng đã chọn không?`, showCancelButton: true, confirmButtonText: 'Xóa', cancelButtonText: 'Hủy', heightAuto: false }); if (!result.isConfirmed) return;
                            for (const record of selected) await services.deleteMainData(record.id, StorageManager.getUserId()); await methods.populateMainData(); mainGrid.refresh(); Swal.fire({ icon: 'success', title: 'Đã xóa', text: `Đã xóa ${selected.length} phiếu giao hàng.`, heightAuto: false }); return;
