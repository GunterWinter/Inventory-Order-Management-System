    ...
    setup() {
        ...
        const services = {
            ...
            createMainData: async (name, description, createdById) => {
                const response = await AxiosManager.post('/CashCategory/CreateCashCategory', {
                    name, description, createdById
                });
                return response;
            },
            updateMainData: async (id, name, description, updatedById) => {
                const response = await AxiosManager.post('/CashCategory/UpdateCashCategory', {
                    id, name, description, updatedById
                });
                return response;
            },
            deleteMainData: async (id, deletedById) => {
                const response = await AxiosManager.post('/CashCategory/DeleteCashCategory', {
                    id, deletedById
                });
                return response;
            },
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Edit Cash Category';
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['CashCategories']);
                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Add Cash Category';
                            resetFormState();
                            mainModal.obj.show();
                        }
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const r = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Delete Cash Category?';
                                state.id = r.id ?? '';
                                state.name = r.name ?? '';
                                state.description = r.description ?? '';
                                mainModal.obj.show();
